    ...
    public static AppChangeHistory generateAppChangeHistories(MarketApp app, MarketAppDTO dto) {
        ...
        if (app.getId() <= 0) {
            ...
            if (!app.getAppName().equals(dto.getAppFullName())) {
                AppChangeDetails details = new AppChangeDetails("名称", app.getAppName() + " -> " + dto.getAppFullName());
                history.addChangeDetails(details);
            }
            if (app.getAppVersionInt() != Integer.valueOf(dto.getAppVersionInt())) {
                AppChangeDetails details = new AppChangeDetails("数字版本", app.getAppVersionInt() + " -> " + dto.getAppVersionInt());
                history.addChangeDetails(details);
            }
            if (!app.getAppVersion().equals(dto.getAppVersion())) {
                AppChangeDetails details = new AppChangeDetails("版本", app.getAppVersion() + " -> " + dto.getAppVersion());
                history.addChangeDetails(details);
            }
            if (!app.getAppPackage().equals(dto.getAppPackage())) {
                AppChangeDetails details = new AppChangeDetails("包名", app.getAppPackage() + " -> " + dto.getAppPackage());
                history.addChangeDetails(details);
            }
            if (app.isRecommend() != dto.isRecommend()) {
                AppChangeDetails details = new AppChangeDetails("推荐", "改变推荐的状态");
                history.addChangeDetails(details);
            }
            ...
            if (app.getAppCategory().getId() != dto.getCategoryId()) {
                ...
                AppChangeDetails details = new AppChangeDetails("类别", app.getAppCategory().getCategoryName() + " -> " + oldCategory.getCategoryName());
                history.addChangeDetails(details);
            }
        }
